...
export function DashboardPage({ records, productItems, selectedId, onSelect, onMatch, onEdit, onClients, onProducts }: { records: ClientRecord[]; productItems: Product[]; selectedId: string; onSelect: (id: string) => void; onMatch: () => void; onEdit: () => void; onClients: () => void; onProducts: () => void }) {
  ...
  const activeResult = matches.find((result) => result.product.id === selectedProductId) ?? matches[0];
  ...
  return <div className="page dashboard-page">
    ...
    <div className="workspace-shell">
      ...
      <section className="matching-workspace">
        ...
        <div className="match-table-block"><div className="section-toolbar"><div><h2>产品匹配结果</h2><p>已按匹配度从高到低排序</p></div><button className="text-button" onClick={onProducts}>进入产品中心<ArrowRight size={14} /></button></div><div className="data-table match-preview-table" role="table"><div className="table-head" role="row"><span>排名</span><span>产品 / 资金方</span><span>预估额度</span><span>利率</span><span>匹配度</span><span>状态</span><span>操作</span></div>{matches.slice(0, 6).map((result, index) => <button className={`table-row ${activeResult.product.id === result.product.id ? "selected" : ""}`} key={result.product.id} onClick={() => setSelectedProductId(result.product.id)} role="row"><span>{index + 1}</span><span><strong>{result.product.name}</strong><small>{result.product.funder}</small></span><span>{result.estimate}</span><span>{result.product.rateLabel}</span><span className="score-cell">{result.score}%</span><span><StatusTag status={result.status} /></span><span className="row-action">查看详情<ArrowRight size={14} /></span></button>)}</div></div>
      </section>
      {activeResult ? <MatchDetail result={activeResult} /> : null}
    </div>
  </div>;
}
